...
export const getChangePriceTxCode = (
...
${options?.preparePreHookCode || ""}
        ...
        assert(self.tokenReceiver.borrow() != nil, message: "Missing or mis-typed FT receiver")
        ...
        assert(self.%nftContract%Provider.check(), message: "Missing or mis-typed HWGaragePackV2 provider")

        let collection = acct.capabilities.borrow<&{NonFungibleToken.Collection}>(collectionData.publicPath)
        ...
        assert(self.%nftContract%Provider.borrow() != nil, message: "Missing or mis-typed Collection provider")
